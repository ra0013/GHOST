...
class DataExtractor
    ...
    def find_database_file(self, extraction_path Path, db_config Dict[str, Any]) - Optional[Path]
        ...
        if self.logger
            self.logger.log_action(DB_SEARCH, fSearching for database {db_config.get('description', 'Unknown')})
        
        # Try primary path
        primary_path = extraction_path  db_config[primary]
        if primary_path.exists()
            if self.logger
                self.logger.log_action(DB_FOUND, fFound at primary path {primary_path})
            return primary_path
        ...
        if backup in db_config
            ...
            if backup_path.exists()
                if self.logger
                    self.logger.log_action(DB_FOUND, fFound at backup path {backup_path})
                return backup_path
        ...
        if alternatives in db_config
            for alt_path in db_config[alternatives]
                ...
                if alt_full_path.exists()
                    if self.logger
                        self.logger.log_action(DB_FOUND, fFound at alternative path {alt_full_path})
            ...
            try
                for candidate in extraction_path.rglob(pattern_parts[-1])
                    if all(part in str(candidate) for part in pattern_parts[-1])
                        if self.logger
                            self.logger.log_action(DB_FOUND, fFound via pattern matching {candidate})
                        return candidate
            except Exception as e
                if self.logger
                    self.logger.log_action(PATTERN_ERROR, fError in pattern matching {str(e)})
        
        if self.logger
            self.logger.log_action(DB_NOT_FOUND, fDatabase not found {db_config['primary']})
        return None
    
    def extract_from_database(self, db_path Path, schema Dict[str, Any]) - List[Dict[str, Any]]
        Extract data using configured schema
        if self.logger
            self.logger.log_action(EXTRACTION_START, fStarting extraction from {db_path})
        ...
        try
            ...
            if self.logger
                self.logger.log_action(QUERY_EXECUTE, fExecuting query {query[200]}...)
            ...
            error_msg = fError extracting from {db_path} {str(e)}
            if self.logger
    ...
    def _build_query_from_schema(self, schema Dict[str, Any]) - str
        ...
        query = fSELECT {', '.join(select_parts)} FROM {table}
    ...
    def extract_sample_data(self, db_path Path, table_name str, limit int = 10) - List[Dict[str, Any]]
        ...
        try
            ...
            cursor.execute(fPRAGMA table_info({table_name}))
            columns = [col[1] for col in cursor.fetchall()]
            
            # Get sample data
            cursor.execute(fSELECT  FROM {table_name} LIMIT {limit})
            results = cursor.fetchall()
            ...
            if self.logger
                self.logger.log_action(SAMPLE_EXTRACT, fExtracted {len(sample_data)} sample records from {table_name})
            
            return sample_data
            
        except Exception as e
            if self.logger
                self.logger.log_action(SAMPLE_ERROR, fError extracting sample from {table_name} {str(e)})
            return []
            ...
            if cursor.fetchone()
                ...
                try
                    ...
                    if test_result
                        ...
                        cursor.execute(fSELECT COUNT() FROM {table})

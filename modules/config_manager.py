...
class ConfigurationManager
    ...
    def load_or_create_config(self, config_file Path, default_config dict) - dict
        ...
        if config_file.exists()
            try
                ...
                print(fError loading {config_file} {e})
                return default_config
        else
            with open(config_file, 'w') as f
                json.dump(default_config, f, indent=2)
            print(fCreated default configuration {config_file})
            return default_config
    ...
    def default_data_paths(self) - dict
        ...
        return {
            messages {
                primary varmobileLibrarySMSsms.db,
                backup varmobileLibrarySMSsms.backup.db,
                description iOS Messages database
            },
            call_history {
                primary varmobileLibraryCallHistoryDBCallHistory.storedata,
                backup varmobileLibraryCallHistoryCallHistory.storedata,
                description iOS Call History database
            },
            contacts {
                primary varmobileLibraryAddressBookAddressBook.sqlitedb,
                backup varmobileLibraryAddressBookAddressBookImages.sqlitedb,
                description iOS Contacts database
            },
            safari {
                primary varmobileLibrarySafariHistory.db,
                backup varmobileContainersDataApplicationLibrarySafariHistory.db,
                description Safari browsing history
            },
            photos {
                primary varmobileMediaPhotoDataPhotos.sqlite,
                backup varmobileMediaDCIM,
    ...
    def default_schemas(self) - dict
        ...
        return {
            messages {
                ...
                timestamp_conversion datetime(date1000000000 + 978307200, 'unixepoch')
            }
        }
